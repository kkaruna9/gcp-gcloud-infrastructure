...
if [ "$san" != "${san#[Yy]}" ] ;then
    ...
    echo Service Account setup is not done.
fi
...
gcloud beta compute ssh --zone $DEFAULT_ZONE_SERVER $DEFAULT_CLUSTER_QEA_SERVER  --project $PROJECT_ID
sudo apt update
...
sudo docker run --rm -ti --name zalenium -p 4444:4444 -v /var/run/docker.sock:/var/run/docker.sock -v /tmp/videos:/home/seluser/videos --privileged dosel/zalenium start
