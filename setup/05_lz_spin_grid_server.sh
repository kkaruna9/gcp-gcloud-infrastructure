...
bash -c external_zaleniumip="";
while [ -z $external_zaleniumip ];
do echo "Please Wait - zalenium grid is Loading...";
external_zaleniumip=$(kubectl get svc release-zalenium --template="{{range .status.loadBalancer.ingress}}{{.ip}}{{end}}");
[ -z "$external_zaleniumip" ] && sleep 15; done; echo "End point ready-" && echo $external_zaleniumip; export endpoint=$external_zaleniumip
